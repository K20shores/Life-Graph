...
exclude=[]
colors = [(key, val) for key, val in mcolors.BASE_COLORS.items() if val not in exclude]
...
class Annotation(Point):
    ...
    def xy_overlapping_width_height(self, that, epsilon):
        ...
        width = min(self.bbox.x1, that.bbox.x1) - max(self.bbox.x0, that.bbox.x0)
        # the coordinates are inverted, so y0 is larger than y1
        height = min(self.bbox.y0, that.bbox.y0) - max(self.bbox.y1, that.bbox.y1)
...
class Lifegraph:
    ...
    def __draw(self):
        ...
        self.ax.plot(xs,ys, color=self.color, marker=self.marker, fillstyle = self.fillstyle, linestyle='none', mew=self.grid_mew)
    ...
    def __format_xaxis(self):
        ...
        self.ax.set_xlabel(r'Week of the Year $\longrightarrow$', color='blue', fontsize=self.fontsize)
    ...
    def __format_yaxis(self):
        ...
        self.ax.set_ylabel(r'$\longleftarrow$ Age', color='blue', fontsize=self.fontsize)
    ...
    def __draw_annotations(self):
        ...
        for a in final:
            if a.draw_point:
                self.ax.plot(a.event_point.x, a.event_point.y, marker='o', color=a.color, markerfacecolor='none', ms=self.annotation_marker_size, mew=self.annotation_edge_width)
    ...
    def __draw_eras(self):
        ...
        for era in self.eras:
            for y in range(era.start.y, era.end.y+1):
                if y == era.start.y:
                    axesUnits = self.ax.transLimits.transform((era.start.x-.5, era.start.y))
                    self.ax.axhspan(y-.5, y+.5, facecolor=era.color, alpha=self.era_alpha, xmin=axesUnits[0], xmax=xmax)
                elif y == era.end.y:
                    axesUnits = self.ax.transLimits.transform((era.end.x+.5, era.end.y))
                    self.ax.axhspan(y-.5, y+.5, facecolor=era.color, alpha=self.era_alpha, xmin=xmin, xmax=axesUnits[0])
                else:
    ...
    def __draw_era_spans(self):
        ...
        for era in self.era_spans:
            ...
            circle2 = plt.Circle((era.end.x, era.end.y), radius, color=era.color, fill=False, lw=self.annotation_edge_width)
        ...
        for unchecked in annotations:
            for checked in final:
                if unchecked.overlaps(checked):
                    correction = unchecked.xy_overlapping_width_height(checked, self.label_space_epsilon)
